...
void putchar(unsigned int *fb, int Xsize, int x, int y, unsigned int FRcolor, unsigned int BKcolor, unsigned char font)
{
	...
	fontp = font_ascii[font];//字符位图
	int testval = 0;

	for(i = 0; i< 16;i++)//这段程序使用到了帧缓存区首地址,将该地址加上字符首像素位置(首像素位置是指字符像素矩阵左上角第一个像素点)的偏移( Xsize * ( y + i ) + x ),可得到待显示字符矩阵的起始线性地址
	{
		addr = fb + Xsize * ( y + i ) + x;
		testval = 0x100;
...
int color_printk(unsigned int FRcolor,unsigned int BKcolor,const char * fmt,...)
{
	int i = 0;
	int count = 0;
	int line = 0;//当前光标距下一个制表位需要填充的空格符数量
	...
	for(count = 0;count < i || line;count++)///通过for语句逐个字符检测格式化后的字符串
	{
		//	add \n \b \t
		if(line > 0)
		{
